...
// VR mode dropdown choices (order matches the vr.c preset table: 0=Diorama, 1=Third-person, 2=First-person,
// 3=Theater). Theater stays out of the dropdown while VR_THEATER_ENABLED is 0 (vr.h) - the count passed to
// the selectionbox below decides what shows.
static char* sVrModeChoices[] = { "Diorama", "Third-person", "First-person", "Theater" };
#define VR_MODE_CHOICE_COUNT (VR_THEATER_ENABLED ? 4 : 3)
...
static unsigned int sTheaterBgMode; // theater backdrop: 0=Black, 1=Panoramic (3D Model is hidden until implemented)
#if VR_THEATER_ENABLED
static char* sTheaterBgChoices[] = { "Black", "Panoramic" };
#endif
...
static unsigned int sTheaterDistI; // theater screen distance, tenths (10..100 = 1.0..10.0 m)
static unsigned int sTheaterSizeI; // theater screen width, tenths (30..200 = 3.0..20.0 m)
...
static struct DjuiSelectionbox *sbMode, *sbTheaterBg;
static struct DjuiSlider   *slMenuDist, *slMenuSize, *slHudSize, *slDioDist, *slDioSize, *slDioHeight, *slStereo, *slWorldScale, *slHead, *slTheaterDist, *slTheaterSize;
...
static void vr_panel_seed_proxies(void) {
    ...
    sTheaterBgMode = (unsigned int)vr_get_theater_bg();
    sMenuDistI  = clampu(vr_get_menu_dist()    * 10.0f,            10, 80);
    sMenuSizeI  = clampu(vr_get_menu_size()    * 10.0f,            20, 120);
    sTheaterDistI = clampu(vr_get_theater_dist() * 10.0f,          10, 100);
    sTheaterSizeI = clampu(vr_get_theater_size() * 10.0f,          30, 200);
...
static void vr_panel_refresh_widgets(void) {
    ...
    if (sbTheaterBg) { djui_selectionbox_update_value(&sbTheaterBg->base); }
    ...
    if (slTheaterDist) { djui_slider_update_value(&slTheaterDist->base); }
    if (slTheaterSize) { djui_slider_update_value(&slTheaterSize->base); }
...
static void vr_panel_mode_changed(UNUSED struct DjuiBase* caller) {
    // 0=Diorama, 1=Third-person, 2=First-person, 3=Theater. First-person also drives the game camera
    // (synced by pc_main). Every mode remembers its own tunables, so after the switch re-seed + redraw
    // the widgets to show the values the mode you just entered actually uses.
    vr_set_preset_index((int)sVrMode);
    vr_panel_seed_proxies();
    vr_panel_refresh_widgets();
}
...
#if VR_THEATER_ENABLED
static void vr_panel_theater_dist_changed(UNUSED struct DjuiBase* caller) { vr_set_theater_dist((float)sTheaterDistI / 10.0f); }
static void vr_panel_theater_size_changed(UNUSED struct DjuiBase* caller) { vr_set_theater_size((float)sTheaterSizeI / 10.0f); }
#endif
...
#if VR_THEATER_ENABLED
static void vr_panel_theaterbg_changed(UNUSED struct DjuiBase* caller) { vr_set_theater_bg((int)sTheaterBgMode); }
#endif
...
void djui_panel_vr_create(struct DjuiBase* caller) {
    ...
    sbTheaterBg = NULL;
    sbMode = NULL;
    slMenuDist = slMenuSize = slHudSize = slDioDist = slDioSize = slDioHeight = slStereo = slWorldScale = slHead = slTheaterDist = slTheaterSize = NULL;
    ...
    {
        if (vr_is_requested()) {
            // VR: too many tunables for one screen (the bottom buttons were getting cut off), so they
            // live in a paginated list - 6 per page, flipped with the < > buttons or the L/R triggers -
            // while Reset to Default and Back stay below the list, always visible.
            struct DjuiPaginated* paginated = djui_paginated_create(body, 6);
            struct DjuiBase* list = &paginated->layout->base;
            // Page 1: view mode + first-person feel toggles.
            sbMode = djui_selectionbox_create(list, "VR Mode", sVrModeChoices, VR_MODE_CHOICE_COUNT, &sVrMode, vr_panel_mode_changed);
            cbFlipCam = djui_checkbox_create(list, "FP Flip Cam (intense)", &sFlipCam, vr_panel_flipcam_changed);
            cbInteractCam = djui_checkbox_create(list, "FP Ease-Back on Interact", &sInteractCam, vr_panel_interactcam_changed);
            cbHideHud = djui_checkbox_create(list, "Hide HUD", &sHideHud, vr_panel_hidehud_changed);
            cbHeadMove  = djui_checkbox_create(list, "Move Where You Look", &sHeadMove, vr_panel_headmove_changed);
            cbAntiClip  = djui_checkbox_create(list, "Camera Anti-Clip", &sAntiClip, vr_panel_anticlip_changed);
            // Page 2: the flat panels (menu, Theater while enabled) + gameplay HUD.
            slMenuDist  = djui_slider_create(list, "Menu Distance",    &sMenuDistI,  10, 80,   vr_panel_menu_dist_changed);
            slMenuSize  = djui_slider_create(list, "Menu Size",        &sMenuSizeI,  20, 120,  vr_panel_menu_size_changed);
#if VR_THEATER_ENABLED
            slTheaterDist = djui_slider_create(list, "Theater Distance", &sTheaterDistI, 10, 100, vr_panel_theater_dist_changed);
            slTheaterSize = djui_slider_create(list, "Theater Size",     &sTheaterSizeI, 30, 200, vr_panel_theater_size_changed);
            sbTheaterBg   = djui_selectionbox_create(list, "Theater Backdrop", sTheaterBgChoices, 2, &sTheaterBgMode, vr_panel_theaterbg_changed);
#endif
            slHudSize   = djui_slider_create(list, "HUD Size",         &sHudSizeI,   10, 60,   vr_panel_hud_size_changed);
            // Page 3: the 3D world (diorama framing, stereo, first-person scale, head damping).
            slDioDist   = djui_slider_create(list, "Diorama Distance", &sDioDistI,   0,  300,  vr_panel_dio_dist_changed);
            slDioSize   = djui_slider_create(list, "Diorama Size",     &sDioSizeI,   30, 3000, vr_panel_dio_size_changed);
            slDioHeight = djui_slider_create(list, "Diorama Height",   &sDioHeightI, 0,  200,  vr_panel_dio_height_changed);
            slStereo    = djui_slider_create(list, "Stereo Depth",     &sStereoI,    0,  200,  vr_panel_stereo_changed);
            slWorldScale = djui_slider_create(list, "World Scale (FP)", &sWorldScaleI, 25, 400, vr_panel_world_scale_changed);
            slHead      = djui_slider_create(list, "Head Motion",      &sHeadI,      0,  150,  vr_panel_head_changed);
            djui_paginated_calculate_height(paginated);
        } else {
            // Flatscreen, where the shrunk-world modes don't apply: just the first-person toggles.
